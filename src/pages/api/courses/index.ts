...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCourse() {
    ...
    if (body?.employee_course?.length > 0) {
      const create_employee_course = body.employee_course;
      body.employee_course = {
        create: create_employee_course,
      };
    } else {
      delete body.employee_course;
    }
