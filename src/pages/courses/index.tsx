...
function CourseListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<CourseInterface[]>(
    ...
    () =>
      getCourses({
        relations: ['team', 'certificate.count', 'employee_course.count', 'video.count'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('certificate', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>certificate</Th>
                  )}
                  {hasAccess('employee_course', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>employee_course</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('certificate', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.certificate}</Td>
                    )}
                    {hasAccess('employee_course', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.employee_course}</Td>
                    )}
